...
function initialFilter(): Filter {
	const requested =
		typeof window === "undefined" ? undefined : window.ryu?.context?.filter;
	return requested === "favorites" || requested === "recent"
		? requested
		: "all";
}
		...
		>
			<DialogContent className="bookmark-dialog sm:max-w-[460px]">
				...
				<form className="bookmark-form" onSubmit={submit}>
					...
					{formError ? (
						<p className="form-error text-status-destructive" role="alert">
							{formError}
						</p>
					) : null}
	...
	return (
		<Empty className="min-h-64">
			<EmptyHeader>
				...
				<EmptyDescription>
					Add a bookmark or import an HTML export to start your library.
				</EmptyDescription>
			</EmptyHeader>
